...
def main():
    ...
    items_to_avoid = np.ndarray([USERS_SIZE, 101], dtype=int)
    for user in range(USERS_SIZE):
        items_to_avoid[user] = dataset_extractor.get_not_interacted_items_for_recommendation(user)
    ...
    # Testing
    top_k = 20
    not_interacted_items = items_to_avoid
    return evaluate_recsys(net, top_k, test_items, not_interacted_items)
